...
from getOID import Caja
from Pract1.setSNMP import setSNMP
from Pract1.getSNMP import consultaSNMP
from Pract1.rrdCreate import rrdBase
import _thread
import time
...
class ComboBoxWindow(Gtk.Window):
    def __init__(self, arbol):
        ...
        _thread.start_new_thread(self.threadGrafica, ())
        _thread.start_new_thread(self.threadSetImagen, ())
    ...
    def Agregar(self, nodo):
        ...
        self.pilaBoxes.append(Caja.Caja())
    ...
    def consulta(self, button):
        if self.info:
            self.info.set_text(consultaSNMP(self.EntryCom.get_text(), self.EntryIP.get_text(),
                                            self.TextOid.get_text()))  # Set default text
    ...
    def on_name_combo_changed(self, combo):
        ...
        if tree_iter is not None:
            ...
            if getNodo.hijos:
                ...
                self.info = Gtk.Entry()
                self.info.set_text(
                    consultaSNMP(self.EntryCom.get_text(), self.EntryIP.get_text(), row_id))  # Set default text
                ...
                self.buttonUpdate.connect("clicked", self.update, self.EntryCom.get_text(), self.EntryIP.get_text(),
                                          row_id)
